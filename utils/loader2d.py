...
def read_data(folder_name):
    ...
    num_files = len([name for name in os.listdir('data2d/'+folder_name) if os.path.isfile(os.path.join('data2d/'+folder_name, name))])

    blur_data = np.zeros((num_files,128,128,128))
    gt_data = np.zeros((num_files,128,128,128))

    for i in range(1, num_files+1):
        x_nuc = tifffile.imread('data2d/'+folder_name+'/'+str(i)+'.tif', maxworkers=6)
        x_gt = tifffile.imread('data2d/gt/0_'+str(i)+'.tif', maxworkers=6)
